...
pub fn app_group_container_path() -> Option<PathBuf> {
    ...
    // Uncaged (the Oss channel) is not signed into Warp's Apple Developer team and
    // has no app-group entitlement. Probing Warp's group container
    // (`~/Library/Group Containers/<team>.dev.warp`) is both wrong for a fork and
    // unsafe: the writability check below does a file `open()` inside that
    // container, and if the directory is inaccessible or wedged (e.g. left in a
    // bad state after the disk filled up), that `open()` blocks forever and hangs
    // startup before any window is shown. Skip it entirely — callers fall back to
