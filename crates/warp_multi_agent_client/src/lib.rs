...
pub enum Error {
    ...
    /// An error from the local Uncaged engine (bring-your-own-model backend).
    #[cfg(not(target_family = "wasm"))]
    #[error("Local Uncaged engine error")]
    LocalEngine(#[source] anyhow::Error),
}
...
pub async fn generate_multi_agent_output(
    ...
    // Uncaged seam: if the user has configured a local bring-your-own-model
    // engine, drive Agent Mode from it instead of POSTing to Warp's servers.
    // Everything above and below this function is unchanged — the client's
    // request building, response dispatch, and native tool execution all run as
    // normal, only the inference source moves onto the user's machine.
    #[cfg(not(target_family = "wasm"))]
    if let Some(local_stream) = uncaged_engine::run_if_active(request) {
        tracing::info!("Uncaged: routing Agent Mode turn to local engine");
        let mapped = local_stream.map(|event| event.map_err(Error::LocalEngine));
        return Ok(mapped.boxed());
    }

    // Uncaged: NEVER fall back to Warp's servers. On the Oss channel the local
    // engine is the only inference source — if it isn't active, hard-fail with a
    // clear message instead of POSTing the prompt to app.warp.dev.
    #[cfg(not(target_family = "wasm"))]
    if matches!(ChannelState::channel(), warp_core::channel::Channel::Oss) {
        return Err(Error::LocalEngine(anyhow::anyhow!(
            "No model is connected. Open Settings \u{2192} AI Models and connect a model \
             (an API key, a local runtime like LM Studio or Ollama, or a CLI agent) \
             to use the agent — Uncaged never sends your prompts to a remote server."
        )));
    }
