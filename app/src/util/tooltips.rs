    ...
    let tooltip_element = if is_enterprise_secret_redaction_enabled {
        let tooltip_column = Flex::column()
            ...
            .with_child(
                appearance
                    .ui_builder()
                    .span("*Secrets are stored locally on your device.")
                    .with_style(UiComponentStyles {
                        font_size: Some(12.),
...
///
/// This checks:
/// - Whether Uncaged is already the default editor (skip if so)
/// - Whether this file is openable in Warp (skips binary files and directories)
/// - Whether Uncaged is an OS-level default editor (skips Markdown files)
#[cfg(feature = "local_fs")]
