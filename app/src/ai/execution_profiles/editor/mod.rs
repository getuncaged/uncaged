...
impl ExecutionProfileEditorView {
        ...
        D: FnOnce(&LLMPreferences) -> LLMId,
    {
        menu.update(ctx, |dropdown, ctx| {
            ...
            // Uncaged: no plans or upgrades — never show the "unavailable on free
            // plans. Upgrade" footer. Every model the user connects is available.
            let _ = (&has_upgrade_gated_models, &upgrade_mouse_state);
