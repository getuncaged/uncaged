...
impl CloudSetupGuideView {
    ...
    fn render_header(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let subtitle = Text::new(
            "Start cloud agents directly in Uncaged from an integration (Linear, Slack), with an event (GitHub, built-in schedule), or programmatically.",
            appearance.ui_font_family(),
            subtitle_font_size,
        )
    ...
    fn render_quick_start_banner(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let text = Text::new_inline(
            "Quick start: configure agents from the command palette.",
            appearance.ui_font_family(),
            font_size,
        )
        .with_style(Properties::default().weight(Weight::Semibold))
    ...
    fn render_manual_setup_header(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        Text::new(
            "Manual setup: Create a Slack or Linear integration with the Uncaged CLI",
            appearance.ui_font_family(),
            font_size,
        )
        .with_style(Properties::default().weight(Weight::Semibold))
    ...
    fn render_step_1(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        ...
        let sub_description = Container::new(Self::render_description_with_link(
            "Use Uncaged's environment setup command to have an agent help you through it. ",
            "Visit docs",
