...
pub(crate) fn render_orchestrator_avatar_disc(
    ...
    // Show the logo of the model/platform currently answering; fall back to the
    // Uncaged mark when the local engine isn't driving. See app/src/uncaged.rs.
    let glyph = crate::uncaged::active_provider_icon().unwrap_or(Icon::AiCaret);
    render_avatar_disc(
        theme.ansi_fg_cyan(),
        AvatarGlyph::Icon(glyph),
        size,
        theme,
        appearance,
    )
...
impl OrchestrationPillBar {
    ...
    fn pill_specs(&self, app: &AppContext) -> Option<(AIConversationId, Vec<PillSpec>)> {
        ...
        specs.push(PillSpec {
            ...
            avatar_glyph: AvatarGlyph::Icon(crate::brand::design::AGENT),
