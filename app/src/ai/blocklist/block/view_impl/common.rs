...
const INTERNAL_WARP_ERROR: &str = "Internal error.";
...
pub fn render_warping_indicator_base(
    ...
    // The Uncaged caret (`›`, U+203A) that prefixes loading text. Replaces Warp's
    // private-use logo glyph (U+E500) from the bundled Roboto; U+203A is present in
    // Roboto (unlike the dingbat `❯` U+276F) so it renders without system fallback.
    // Below, a transparent copy is used only to indent tips to match the caret width.
...
pub fn render_failed_output(props: FailedOutputProps, app: &AppContext) -> Box<dyn Element> {
    ...
    let error_text = match props.error {
        ...
        RenderableAIError::ServerOverloaded => {
            "The model is currently overloaded. Please try again later.".to_string()
        }
...
pub fn render_user_avatar(
    ...
    let avatar = Avatar::new(
        profile_image_path
            ...
            // Uncaged has no accounts, so a name initial is meaningless — show a
            // neutral "you" glyph for the human turn instead of a stray letter.
            .unwrap_or(AvatarContent::Icon(Icon::User)),
        UiComponentStyles {
