...
const TOS_URL: &str = crate::brand::HOME_URL;
...
impl LoginSlideView {
    ...
    fn render_select_auth_content(&self, appearance: &Appearance) -> Vec<Box<dyn Element>> {
        ...
        let (title_text, subtitle_text) = match self.login_purpose() {
            ...
            LoginPurpose::ThirdParty => (
                "Create an account",
                "Uncaged is free and needs no account. Connect your own model (API, local model, or CLI) in Settings to power the agent.",
            ),
        };
        ...
        let tos_line = Flex::row()
            .with_child(
                ui_builder
                    .span("By continuing, you agree to Uncaged's ")
    ...
    fn render_skip_dialog(&self, appearance: &Appearance) -> Box<dyn Element> {
        let (title, body, features, cancel_label): (
            ...
            LoginPurpose::WarpAgent | LoginPurpose::ThirdParty => (
                ...
                "Uncaged's AI runs entirely on your own model, no account required. Set it up in Settings, AI.",
                &[],
                "Sign in",
            ),
        };
