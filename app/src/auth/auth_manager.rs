...
impl AuthManager {
    ...
    pub fn initialize_user_from_auth_payload(
        ...
        // Uncaged: no accounts. Refuse to complete any Warp login on the Oss
        // channel, so no credentials are ever stored and no login-gated egress to
        // app.warp.dev can occur — even via a crafted uncagedoss:// callback.
        if matches!(
            warp_core::channel::ChannelState::channel(),
            warp_core::channel::Channel::Oss
        ) {
            log::info!("Uncaged: ignoring auth payload (accounts are disabled).");
            return;
        }
