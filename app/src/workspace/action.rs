...
pub enum WorkspaceAction {
    ...
    /// Install the Uncaged CLI command to /usr/local/bin
    #[cfg(target_os = "macos")]
    InstallOz,
    /// Uninstall the Uncaged CLI command from /usr/local/bin
    #[cfg(target_os = "macos")]
    UninstallOz,
    ...
    OpenSettingsFile,
    /// Exports portable settings + workflows to a chosen folder (secrets excluded).
    BackUpSettings,
    /// Restores portable settings from a chosen backup archive.
    RestoreSettings,
    /// Pushes the portable config + Drive bundle to a private GitHub gist via
    /// the system `gh` CLI (user-initiated, opt-in; no token stored).
    PushConfigToGist,
    ...
    OpenSshConfig,
    /// Opens a new terminal tab and launches the given third-party CLI agent
    /// (e.g. `claude`, `codex`) by running its command in a fresh shell.
    LaunchCliAgent {
        agent: crate::terminal::cli_agent::CLIAgent,
    },
    /// Opens a config file/dir from the Config panel: files open in an editor
    /// tab, directories are revealed in the file manager.
    OpenConfigFile {
