...
use warpui::elements::{
    resizable_state_handle, ChildView, ClippedScrollStateHandle, ConstrainedBox, Container,
...
struct MouseStateHandles {
    ...
    ssh_button: MouseStateHandle,
    skills_button: MouseStateHandle,
    config_button: MouseStateHandle,
}
...
impl LeftPanelView {
    pub fn new(
        ...
        let ssh_hosts = ssh_panel::read_ssh_hosts();
        let ssh_row_states = ssh_hosts
            .iter()
        ...
        let mut view = Self {
            ...
            config_entries,
            config_row_states,
            config_action_states: Default::default(),
            config_scroll_state: Default::default(),
    ...
    fn create_toolbelt_button_config(
        ...
        match view {
            ...
            ToolPanelView::Ssh => {
                let tooltip_keybinding_names = vec![];
                ToolbeltButtonConfig {
                    icon: crate::brand::design::SSH,
                    active_icon: None,
                    tooltip_text: "SSH hosts".to_string(),
                    action: LeftPanelAction::Ssh,
                    render_with_active_state: false,
                    tooltip_keybinding: toolbelt_tooltip_keybinding(&tooltip_keybinding_names, ctx),
                    tooltip_keybinding_names,
                }
            ...
            ToolPanelView::Skills => {
                let tooltip_keybinding_names = vec![];
                ToolbeltButtonConfig {
                    icon: crate::brand::design::SKILLS,
                    active_icon: None,
            ...
            ToolPanelView::Config => {
                let tooltip_keybinding_names = vec![];
                ToolbeltButtonConfig {
                    icon: crate::brand::design::CONFIG,
                    active_icon: None,
                    tooltip_text: "Config".to_string(),
                    action: LeftPanelAction::Config,
                    render_with_active_state: false,
                    tooltip_keybinding: toolbelt_tooltip_keybinding(&tooltip_keybinding_names, ctx),
                    tooltip_keybinding_names,
                }
    ...
    /// Re-reads `~/.ssh/config` so hosts added since the panel was created show
    /// up. Called every time the SSH panel is entered.
    fn refresh_ssh_hosts(&mut self, ctx: &mut ViewContext<Self>) {
    ...
    /// Re-reads the config items on disk (e.g. after a restore). Called every
    /// time the Config panel is entered.
    fn refresh_config_entries(&mut self, ctx: &mut ViewContext<Self>) {
        self.config_entries = config_panel::read_config_entries();
        self.config_row_states = self
            .config_entries
            .iter()
            .map(|_| MouseStateHandle::default())
            .collect();
    ...
    fn update_button_active_states(&mut self) {
        for button in &mut self.toolbelt_buttons {
            button.render_with_active_state = match &button.action {
                ...
                LeftPanelAction::Ssh => self.active_view.get() == ToolPanelView::Ssh,
                LeftPanelAction::Skills => self.active_view.get() == ToolPanelView::Skills,
                LeftPanelAction::Config => self.active_view.get() == ToolPanelView::Config,
            };
        }
    }
...
impl LeftPanelView {
    pub fn handle_action_with_force_open(
        ...
        match action {
            ...
            LeftPanelAction::Ssh => {
                active_view_state::set(self, ToolPanelView::Ssh, ctx);
            }
            LeftPanelAction::Skills => {
                active_view_state::set(self, ToolPanelView::Skills, ctx);
            }
            LeftPanelAction::Config => {
                active_view_state::set(self, ToolPanelView::Config, ctx);
            }
        }
    }
...
impl View for LeftPanelView {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let mouse_state_handles = vec![
            ...
            self.mouse_state_handles.ssh_button.clone(),
            self.mouse_state_handles.skills_button.clone(),
            self.mouse_state_handles.config_button.clone(),
        ];
        ...
        let content_area: Box<dyn Element> = match self.active_view.get() {
            ...
            ToolPanelView::Ssh => Shrinkable::new(
                1.0,
                ssh_panel::render_ssh_content(
                    &self.ssh_hosts,
                    &self.ssh_row_states,
                    &self.ssh_add_button_state,
                    &self.ssh_scroll_state,
                    appearance,
                ),
            )
            .finish(),
            ToolPanelView::Skills => Shrinkable::new(
                1.0,
            ...
            .finish(),
            ToolPanelView::Config => Shrinkable::new(
                1.0,
                config_panel::render_config_content(
                    &self.config_entries,
                    &self.config_row_states,
                    &self.config_action_states,
                    &self.config_scroll_state,
                    appearance,
                ),
            )
            .finish(),
