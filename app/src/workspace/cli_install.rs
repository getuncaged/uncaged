...
/// Compute the target path where the Uncaged CLI symlink should be installed, based on channel
...
///
/// Uncaged commands are part of the shared executable's normal argument parser, so
...
fn create_symlink_with_admin(source: &Path, target: &Path) -> Result<()> {
    ...
    let script = format!(
        "do shell script \"ln -sf {escaped_source} {escaped_target}\" with prompt \"Uncaged needs administrator privileges to install the command in /usr/local/bin.\" with administrator privileges"
    );
...
fn remove_file_with_admin(target: &Path) -> Result<()> {
    ...
    let script = format!(
        "do shell script \"rm {escaped_target}\" with prompt \"Uncaged needs administrator privileges to uninstall the command from /usr/local/bin.\" with administrator privileges"
    );
...
/// Install the Uncaged CLI by symlinking the shared Warp executable into /usr/local/bin.
///
...
pub fn install_oz() -> Result<()> {
    ...
    let current_binary =
        std::env::current_exe().context("Failed to get current executable path")?;
    install_symlink(&current_binary, &oz_path, "Uncaged CLI")
}

/// Uninstall the Uncaged CLI by removing the symlink from /usr/local/bin
pub fn uninstall_oz() -> Result<()> {
    uninstall_symlink(&oz_install_target_path(), "Uncaged command")
}
...
pub fn install_warpctrl() -> Result<()> {
    ...
    if !warpctrl_source.exists() {
        return Err(anyhow!(
            "Cannot install Uncaged Control CLI: bundled wrapper not found at {}",
            warpctrl_source.display()
        ));
    }

    install_symlink(&warpctrl_source, &warpctrl_path, "Uncaged Control CLI")
}

/// Uninstall the Warp Control CLI by removing the symlink from /usr/local/bin
pub fn uninstall_warpctrl() -> Result<()> {
    uninstall_symlink(&warpctrl_install_target_path(), "Uncaged Control command")
}
