...
impl HeaderToolbarItemKind {
    ...
    pub fn default_left() -> Vec<Self> {
        // Uncaged: the Agent Management panel's zero-state is a Warp cloud-agent
        // (Oz) setup guide that doesn't apply to a local, cloud-free fork, so it
        // is not shown in the default toolbar.
