...
impl UserWorkspaces {
    ...
    // The owner uid used for the personal drive. In the account-free Uncaged
    // build there is usually no real signed-in user, so we fall back to a stable
    // local placeholder uid. Stability matters: local objects persist their
    // owner into sqlite, so if this changed across restarts old objects would no
    // longer resolve to the Personal space and would appear to vanish.
    fn effective_personal_user_uid(ctx: &AppContext) -> UserUid {
        AuthStateProvider::as_ref(ctx)
            .get()
            .user_id()
            .unwrap_or_else(|| UserUid::new(TEST_USER_UID))
    }

    // Returns the [`Owner`] for the user's personal drive. Uncaged is
    // account-free, so this always yields an owner (using a stable local
    // placeholder uid when no real user is signed in); local create/import
    // actions rely on this being `Some`.
    ...
    pub fn owner_to_space(&self, owner: Owner, ctx: &AppContext) -> Space {
        match owner {
            Owner::User { user_uid } => {
                ...
                // Compare against the effective personal uid so that objects
                // owned by the account-free local placeholder still resolve to
                // the Personal space (not Shared).
