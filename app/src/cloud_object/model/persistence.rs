...
pub struct CloudModel {
    ...
    /// Signals that the initial local load of cached objects is complete and
    /// the store is readable. Unlike the server-driven `UpdateManager` initial
    /// load (which never fires when logged out), this is satisfied as soon as
    /// the local sqlite restore has populated the model — which is exactly the
    /// point at which `CloudModel::new` runs. UI (e.g. the Warp Drive panel)
    /// awaits this so it doesn't spin forever in the account-free Uncaged build.
...
impl CloudModel {
    pub fn new(
        ...
        // The cached objects passed in above are the result of the local sqlite
        // restore, so by the time we construct the model the local load is done.
    ...
    /// Wait for the initial local load to complete. In the account-free build
    /// this resolves as soon as the local sqlite restore has run (see
    /// [`CloudModel::new`]).
    pub fn initial_load_complete(&self) -> impl Future<Output = ()> {
        // Not an `async fn` so the returned Future doesn't borrow `self`.
        self.initial_load_complete.wait()
    }

    /// Mark the initial local load as complete.
    ...
    pub fn reset(&mut self) {
        ...
        // The local store is immediately readable after a reset (it's just
        // empty), so keep the initial-load signal satisfied.
        self.initial_load_complete.set();
    }
}
