...
pub async fn fetch_channel_versions(
    ...
    // Uncaged: the Oss channel has no update server and must never contact
    // app.warp.dev — refuse the version check outright (belt-and-suspenders with
    // the start_polling gate).
    if matches!(ChannelState::channel(), Channel::Oss) {
        anyhow::bail!("Autoupdate is disabled on the Uncaged channel");
    }
