...
pub enum CommandExecutionSource {
    /// A non-shared command execution request from Uncaged AI++.
...
pub enum InputAction {
    ...
    /// Attach the most recent terminal block and ask the agent to diagnose/fix it.
    FixLastCommandWithAi,
...
pub fn init(app: &mut AppContext) {
    ...
    app.register_editable_bindings([
        ...
        // On-demand "Fix with AI": attach the most recent block and ask the agent
        // to diagnose/fix it. Palette-discoverable; no default key to avoid clashes.
        EditableBinding::new(
            FIX_WITH_AI_KEYBINDING_NAME,
            "Fix last command with AI",
            InputAction::FixLastCommandWithAi,
        )
        .with_group(bindings::BindingGroup::WarpAi.as_str())
        .with_context_predicate(
            id!("Input") & id!(flags::IS_ANY_AI_ENABLED) & id!("TerminalView_NonEmptyBlockList"),
...
impl Input {
    ...
    fn open_model_selector_and_snapshot_prompt(
        ...
        // Uncaged: the local engine — not Warp's hosted roster — decides which
        // model runs Agent Mode. Open the AI Models gallery (our model manager,
        // where models are connected and switched) instead of a picker full of
        // Warp models that would be ignored.
    ...
    /// On-demand "Fix with AI": pin the most recent block as agent context and ask
    /// the agent to diagnose/fix it. Reuses the pending-context + submit path, so
    /// the attached block reaches the model exactly like manual attachment does.
    fn fix_last_command_with_ai(&mut self, ctx: &mut ViewContext<Self>) {
        let last_block_id = {
            let model = self.model.lock();
            model
                .block_list()
    ...
    fn agent_mode_hint_text(&mut self, app: &AppContext) -> String {
        ...
        match (
            ...
            (InputType::AI, _) => {
                ...
                //  * `None` (no conversation, empty, passive, or untitled) => new conversation => "Ask anything"
...
impl TypedActionView for Input {
    ...
    fn handle_action(&mut self, action: &InputAction, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            InputAction::FixLastCommandWithAi => {
                self.fix_last_command_with_ai(ctx);
            }
