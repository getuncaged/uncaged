        ...
        let menu_view = if FeatureFlag::InlineMenuHeaders.is_enabled() {
            ...
            ctx.add_typed_action_view(|ctx| {
                ...
                menu.with_banner_fn(move |app| {
                    // With the local Uncaged engine active, this Warp roster is
                    // inert — Agent Mode runs the connected model regardless of
                    // what's picked here. Say so instead of implying otherwise.
                    if crate::uncaged::engine_active() {
                        let appearance = Appearance::as_ref(app);
                        return Some(Alert::new().render(
                            AlertConfig::new(
                                "Uncaged runs your connected model — choose it in Settings › AI Models."
                                    .to_string(),
                                crate::view_components::alert::AlertFlavor::Default,
                            )
