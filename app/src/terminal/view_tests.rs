...
fn test_link_at_range_trims_zero_width_spaces() {
    App::test((), |mut app| async move {
        ...
        let input_url = "(\u{200b}https://github.com/uncaged-terminal/uncaged)";
        // NOTE: the final character in this string is a zero-width space
        let non_escaped_url = "https://github.com/uncaged-terminal/uncaged";
        let escaped_url = "https://github.com/uncaged-terminal/uncaged";

        terminal.update(&mut app, |view, _ctx| {
            view.model.lock().simulate_block(
                r"printf '(%bhttps://github.com/uncaged-terminal/uncaged)\n' '\U200b' '\U200b'",
                input_url,
            );
        });
