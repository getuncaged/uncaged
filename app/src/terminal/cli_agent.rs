...
impl CLIAgent {
    ...
    /// The binary name to invoke when launching this agent from the CLI-agent
    /// launcher bar. Differs from [`command_prefix`](Self::command_prefix) only
    /// for Cursor, whose installed binary is `cursor-agent` (the bare `agent`
    /// prefix is used only for in-terminal detection).
    pub fn launch_command(&self) -> &'static str {
        match self {
            CLIAgent::CursorCli => "cursor-agent",
            other => other.command_prefix(),
        }
    }
...
//
// Uncaged: a fully-local scan of $PATH (plus the bin dirs GUI apps commonly
// miss — homebrew/cargo/bun/local/nvm) for each known CLI agent's binary, so
// the "Third party CLI agents" settings page can show which agents you actually
// have installed. Zero network, no subprocess (we stat files rather than shell
// out, so there's no window flash), no telemetry.
...
impl CLIAgentInstallModel {
    ...
    /// `false` until the scan completes.
    pub fn is_installed(&self, agent: CLIAgent) -> bool {
        self.installed
            .as_ref()
    ...
    /// Whether the background scan has finished — the UI shows a "checking…"
...
/// Scans `$PATH` + common bin dirs for each known agent's binary. Pure
/// filesystem stats — no network, no subprocess.
fn scan_cli_agent_installations() -> HashMap<CLIAgent, bool> {
    let dirs = cli_agent_search_dirs();
    enum_iterator::all::<CLIAgent>()
        .filter(|a| !matches!(a, CLIAgent::Unknown))
        .map(|a| (a, cli_agent_binary_on_path(a, &dirs)))
        .collect()
}

fn cli_agent_binary_on_path(agent: CLIAgent, dirs: &[PathBuf]) -> bool {
    match agent {
        CLIAgent::Unknown => false,
        // Cursor's installed binary is `cursor-agent`, not the generic `agent`
        // command prefix used for in-terminal detection.
        CLIAgent::CursorCli => binary_in_dirs("cursor-agent", dirs),
        other => binary_in_dirs(other.command_prefix(), dirs),
    }
}

fn binary_in_dirs(cmd: &str, dirs: &[PathBuf]) -> bool {
    !cmd.is_empty() && dirs.iter().any(|dir| dir.join(cmd).is_file())
}

/// `$PATH` entries plus the common bin dirs a GUI-launched app often can't see
/// (homebrew, cargo, bun, ~/.local, and each installed nvm node version).
fn cli_agent_search_dirs() -> Vec<PathBuf> {
