...
pub enum ProfileModelSelectorAction {
    ...
    /// Uncaged: activate a saved local-engine connection by id.
    SelectUncagedConnection(String),
    /// Uncaged: switch the active connection's model, then re-activate it.
    SelectUncagedModel {
...
impl ProfileModelSelector {
    /// When the local Uncaged engine is active it — not Warp's server roster —
    /// decides which model runs Agent Mode. In that case the footer chip should
    /// state the actually-connected model rather than a Warp model like
    /// "auto (genius)". Returns `None` when the engine is dormant (fall back to
    /// the normal label).
    fn uncaged_model_label() -> Option<String> {
        if !crate::uncaged::engine_active() {
            return None;
        }
        crate::uncaged::connections()
            .into_iter()
            .find(|c| c.is_active)
            .map(Self::uncaged_connection_label)
    }

    /// A short label for one Uncaged connection: its model if set, else its
    /// provider name (e.g. "LM Studio" when no model id is configured).
    fn uncaged_connection_label(c: crate::uncaged::ConnectionView) -> String {
    ...
    /// Show a brief toast in the current window. Used to confirm Uncaged model /
    /// connection switches (which happen without any page navigation).
    fn show_toast(&self, message: String, ctx: &mut ViewContext<Self>) {
        let window_id = ctx.window_id();
        crate::workspace::ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
            toast_stack.add_ephemeral_toast(
                crate::view_components::DismissibleToast::default(message),
                window_id,
                ctx,
            );
        });
    ...
    /// Populate the model dropdown for the Uncaged local engine: one row per
    /// saved connection (`"{label} · {model}"`, active one checked), and —
    /// indented under the active connection — one row per selectable model of
    /// that connection's platform. Selecting a connection activates it;
    /// selecting a model edits+re-activates the active connection.
    fn refresh_uncaged_model_menu(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        // Index of the active connection's own row, so the menu opens focused
        // on it.
        let mut active_index = 0usize;

        for conn in &connections {
            let model_label = if conn.model.trim().is_empty() {
                "no model".to_string()
            } else {
                conn.model.clone()
            };
            let mut fields = MenuItemFields::new(format!("{} · {}", conn.label, model_label))
                .with_on_select_action(ProfileModelSelectorAction::SelectUncagedConnection(
                    conn.id.clone(),
                ));
            if conn.is_active {
            ...
            // Under the active connection, list its selectable models so one
            // click switches the specific model. Skip when there's nothing to
            // choose between (a single model equal to the current one).
            if conn.is_active {
                let show_models = conn.models.len() > 1
                    || (conn.models.len() == 1 && conn.models[0] != conn.model);
                if show_models {
                    for model in &conn.models {
                        let is_current = model == &conn.model;
                        let mut model_fields = MenuItemFields::new(format!("    {model}"))
                            .with_override_text_color(sub_color)
                            .with_on_select_action(
                                ProfileModelSelectorAction::SelectUncagedModel {
                                    connection_id: conn.id.clone(),
                                    model: model.clone(),
                                },
                            );
                        if is_current {
                            model_fields = model_fields.with_right_side_icon(Icon::Check);
                        }
                        items.push(MenuItem::Item(model_fields));
                    }
                }
            }
        }
        ...
        if items.is_empty() {
            items.push(MenuItem::Item(
                MenuItemFields::new("No models connected — open AI Models to add one")
                    .with_override_text_color(sub_color)
        ...
        self.model_dropdown.update(ctx, |menu, ctx| {
            ...
            menu.set_selected_by_index(active_index, ctx);
            ctx.notify();
        });
    }
    ...
    fn refresh_state(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let model_name = if let Some(label) = Self::uncaged_model_label() {
            label
    ...
    fn refresh_model_menu(&mut self, ctx: &mut ViewContext<Self>) {
        if crate::uncaged::engine_active() {
            // The Uncaged menu is populated on open (and kept correct here so a
            // background refresh while it's open doesn't clobber it with Warp
            // models).
            self.refresh_uncaged_model_menu(ctx);
            return;
        }
    ...
    fn render_model_section(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let model_display_name = if let Some(label) = Self::uncaged_model_label() {
            label
...
impl TypedActionView for ProfileModelSelector {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            ProfileModelSelectorAction::SelectUncagedConnection(id) => {
                self.set_model_menu_visibility(false, ctx);
                match crate::uncaged::activate(id) {
                    Ok(()) => {
                        let label = crate::uncaged::connections()
                            .into_iter()
                            .find(|c| c.is_active)
                            .map(Self::uncaged_connection_label)
                            .unwrap_or_else(|| "model".to_string());
                        self.show_toast(format!("Switched to {label}"), ctx);
                    }
                    Err(e) => self.show_toast(format!("Couldn't switch: {e}"), ctx),
                }
                ctx.notify();
            }
            ProfileModelSelectorAction::SelectUncagedModel {
                ...
                self.set_model_menu_visibility(false, ctx);
                // Find the connection so we can persist an edit that only
                // changes the model, leaving name/url/key/cli_command intact.
                let existing = crate::uncaged::connections()
                    .into_iter()
                    .find(|c| &c.id == connection_id);
                let result = if let Some(c) = existing {
                    crate::uncaged::update(
                        &c.id,
                        c.label.clone(),
                        c.base_url.clone(),
                        model.clone(),
                        // Some("") = keep the stored key.
                        Some(String::new()),
                        c.cli_command.clone(),
                    )
                    .and_then(|()| crate::uncaged::activate(&c.id))
                } else {
                    Err("connection no longer exists".to_string())
                };
                match result {
                    Ok(()) => self.show_toast(format!("Model set to {model}"), ctx),
                    Err(e) => self.show_toast(format!("Couldn't set model: {e}"), ctx),
                }
                ctx.notify();
            }
            ProfileModelSelectorAction::ToggleModelMenu => {
                ...
                if crate::uncaged::engine_active() {
                    // Uncaged: the local engine — not Warp's roster — owns the
                    // active model, so open an inline menu of the user's
                    // connections and their models instead of the Warp picker.
                    let opening = !self.is_model_menu_open;
                    if opening {
                        self.refresh_uncaged_model_menu(ctx);
                    }
                    self.set_model_menu_visibility(opening, ctx);
                } else if self.is_third_party_harness(ctx) {
