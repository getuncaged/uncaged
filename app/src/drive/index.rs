...
struct MouseStateHandles {
    ...
    gist_sync_button_mouse_state: MouseStateHandle,
}
...
impl DriveIndex {
                ...
                cloud_model
                    ...
                    .filter(move |cloud_object| {
                        cloud_object.renders_in_warp_drive()
                            && (!hide_welcome || !cloud_object.is_welcome_object())
    ...
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        // Hide Uncaged Drive loading icon once initial load is complete.
        // Uncaged is account-free: the server-driven UpdateManager initial load
        // never fires when logged out, so we wait on CloudModel's local-restore
        // signal (satisfied right after the local sqlite restore) instead.
    ...
    fn render_title(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        ...
        // Uncaged: back up the local Drive (+ config) to a private GitHub gist.
        // Reuses the same bundle as the Config panel — drive content is included.
        let gist_sync_button = icon_button(
            appearance,
            Icon::UploadCloud,
            false,
        ...
        .build()
        .on_click(|ctx, _, _| {
            ctx.dispatch_typed_action(crate::workspace::WorkspaceAction::PushConfigToGist)
        })
        .finish();

        title_right_side.add_child(
            Container::new(Align::new(gist_sync_button).finish())
