...
/// What kind of backend this modal is editing. The same form serves both an
/// HTTP API endpoint and a locally-installed CLI agent (Uncaged), but the two
/// have different levers: an API needs a URL + key, a CLI needs its command
/// line (invocation + flags). `Cli` swaps the URL field for a "Command" field,
/// hides the API key, and skips URL validation (a command is not a URL).
...
pub struct CustomEndpointModal {
    ...
    url_has_error: bool,
    /// API endpoint vs. CLI agent — drives the URL/Command relabel, whether the
    /// API-key field shows, and whether the URL is validated.
    kind: EndpointKind,
}
...
impl CustomEndpointModal {
    pub fn new(
        ...
        let endpoint_name_editor = ctx.add_typed_action_view(move |ctx| {
            ...
            editor.set_placeholder_text("e.g. My Claude", ctx);
    ...
    /// Switch the form between API-endpoint and CLI-agent modes. Updates the
    /// URL/Command placeholder to match and clears any stale URL error (a
    /// command is never URL-validated). Call before `open`/`prefill`.
    pub fn set_kind(&mut self, kind: EndpointKind, ctx: &mut ViewContext<Self>) {
        self.kind = kind;
        let placeholder = match kind {
            EndpointKind::Api => "Please include 'https://'",
            EndpointKind::Cli => "e.g. claude-code-acp --model opus",
        };
        self.endpoint_url_editor.update(ctx, |editor, ctx| {
            editor.set_placeholder_text(placeholder, ctx);
        });
        if kind == EndpointKind::Cli {
    ...
    fn save(&mut self, ctx: &mut ViewContext<Self>) {
        // A CLI command is not a URL — never validate it or block Save on it.
        if self.kind == EndpointKind::Api {
    ...
    fn is_valid(&self, app: &AppContext) -> bool {
        ...
        match self.kind {
            EndpointKind::Api => is_endpoint_form_valid(&name, &url, &api_key, has_models),
            // A CLI just needs a name + a command; the model is optional and it
            // has no key or URL to validate.
            EndpointKind::Cli => !name.trim().is_empty() && !url.trim().is_empty(),
        }
    }
    ...
    fn handle_endpoint_url_event(&mut self, event: &EditorEvent, ctx: &mut ViewContext<Self>) {
        // For a CLI the key field is hidden, so Tab/Enter jumps straight to the
        // first model row; validation is API-only (a command is not a URL).
        let after_url = |me: &Self, ctx: &mut ViewContext<Self>| match me.kind {
            EndpointKind::Api => ctx.focus(&me.api_key_editor),
            EndpointKind::Cli => {
                if let Some(first_row) = me.model_rows.first() {
        ...
        match event {
            ...
            EditorEvent::Edited(_) => {
                // A CLI command never enters an error state; only validate an API URL.
                if self.kind == EndpointKind::Api {
...
impl View for CustomEndpointModal {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        // Description — a CLI agent is customized through its command line, not
        // a URL + key, so speak to that.
        let description = match self.kind {
            EndpointKind::Api => {
                "Provide the endpoint and model details. Paste your API key to finish connecting."
            }
            EndpointKind::Cli => {
                "Provide the command for your signed-in CLI agent. Add any flags it supports — model, thinking, etc. — and it'll be passed through."
            }
        };
        column.add_child(
            Container::new(
        ...
        // Endpoint name
        column.add_child(
            Container::new(label("Connection name"))
        ...
        // Endpoint URL — relabeled "Command" for a CLI agent.
        let url_label = match self.kind {
            EndpointKind::Api => "API endpoint",
            EndpointKind::Cli => "Command",
        };
        column.add_child(
            Container::new(label(url_label))
                .with_margin_bottom(4.)
                .finish(),
        );
        ...
        // API key — a CLI agent uses its own login, so it has no key field.
        if self.kind == EndpointKind::Api {
            column.add_child(
        ...
        let mut model_labels = Flex::row()
            ...
            .with_child(
                ConstrainedBox::new(label("Model"))
                    .with_width(MODEL_INPUT_WIDTH)
                    .finish(),
            )
            .with_child(
                ConstrainedBox::new(label("Label (optional)"))
                    .with_width(MODEL_INPUT_WIDTH)
                    .finish(),
            );
...
impl CustomEndpointModalViewState {
    ...
    /// Switch the form between API-endpoint and CLI-agent modes (see
    /// [`EndpointKind`]). Call before `prefill`/`open`.
    pub fn set_kind<T: View>(&mut self, kind: EndpointKind, ctx: &mut ViewContext<T>) {
        self.state.view.update(ctx, |modal, ctx| {
            modal.body().update(ctx, |body, ctx| {
                body.set_kind(kind, ctx);
            });
        });
    }
