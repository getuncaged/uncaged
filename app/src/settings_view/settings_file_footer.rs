...
/// Size of the Oz brand mark inside the "Fix with Uncaged" button. Matches the
/// Figma spec and the workspace banner's secondary-button icon sizing.
const ALERT_OZ_ICON_SIZE: f32 = 14.;
/// Horizontal padding inside the "Open file" / "Fix with Uncaged" action buttons.
...
pub struct SettingsFooterMouseStates {
    ...
    pub back_up_button: MouseStateHandle,
    pub restore_button: MouseStateHandle,
...
/// Renders one full-width, bordered footer action button (leading icon +
/// semibold label) that dispatches `action` on click. Shared by "Open settings
/// file", "Back up settings", and "Restore settings" so they stay visually
/// consistent.
fn render_footer_button(
    ...
    Hoverable::new(mouse_state, move |state| {
        let icon = ConstrainedBox::new(icon.to_warpui_icon(text_fill).finish())
            .with_width(FOOTER_ICON_SIZE)
            .with_height(FOOTER_ICON_SIZE)
            .finish();

        let label = Text::new_inline(label_text, ui_font_family, FOOTER_FONT_SIZE)
...
pub fn render_footer(
    ...
    let inner: Box<dyn Element> = match kind {
        ...
        SettingsFooterKind::OpenButton => Flex::column()
            .with_cross_axis_alignment(CrossAxisAlignment::Center)
            .with_child(render_footer_button(
                appearance,
            ...
            .with_child(
                Container::new(render_footer_button(
                    appearance,
                    mouse_states.back_up_button.clone(),
                    Icon::Download,
                    "Back up settings\u{2026}",
            ...
            .with_child(
                Container::new(render_footer_button(
                    appearance,
                    mouse_states.restore_button.clone(),
                    Icon::Import,
                    "Restore settings\u{2026}",
