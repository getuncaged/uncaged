...
use super::custom_inference_modal::{
    CustomEndpointModal, CustomEndpointModalEvent, CustomEndpointModalViewState, EndpointKind,
};
...
/// Upper bound on how many saved Uncaged connections get pre-allocated
/// per-row action buttons (Use / Edit). Rows beyond this render without those
/// buttons — a roster that large is effectively never hit in practice.
const UNCAGED_MAX_CONN_ROWS: usize = 16;

/// Leading brand-icon size for connection rows and gallery tiles (kept
/// identical so the same provider mark never renders at two sizes on one page).
const UNCAGED_ICON_SIZE: f32 = 18.;
...
pub struct AISettingsPageView {
    ...
    // Uncaged "Connect a model": one "Connect" button per catalog preset (in
    // flattened catalog order), plus fixed pools of per-row "Use"/"Edit"
    // buttons indexed against the live roster. `pending_uncaged_connection_id`
    // routes the shared endpoint modal's save/remove to the roster instead of
    // Warp's custom endpoints while a connection is being configured.
    uncaged_connect_buttons: Vec<ViewHandle<ActionButton>>,
    uncaged_use_buttons: Vec<ViewHandle<ActionButton>>,
    uncaged_edit_buttons: Vec<ViewHandle<ActionButton>>,
    pending_uncaged_connection_id: Option<String>,
...
impl AISettingsPageView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let uncaged_connect_buttons = Self::create_uncaged_connect_buttons(ctx);
        let uncaged_use_buttons = Self::create_uncaged_row_buttons(true, ctx);
        let uncaged_edit_buttons = Self::create_uncaged_row_buttons(false, ctx);
        ...
        Self {
            ...
            uncaged_connect_buttons,
            uncaged_use_buttons,
            uncaged_edit_buttons,
            pending_uncaged_connection_id: None,
    ...
    // ── Uncaged "Connect a model" ──────────────────────────────────────────

    /// One "Connect" button per catalog preset, in flattened catalog order.
    /// Each carries its own preset id, so render can index straight into this
    /// vec in the same order it walks the catalog.
    fn create_uncaged_connect_buttons(
        ctx: &mut ViewContext<Self>,
    ) -> Vec<ViewHandle<ActionButton>> {
        crate::uncaged::catalog_sections()
            .into_iter()
            .flat_map(|section| section.presets)
            .map(|preset| {
    ...
    /// A fixed pool of per-row buttons ("Use" when `is_use`, else "Edit"),
    /// indexed against the live roster at click time.
    fn create_uncaged_row_buttons(
        is_use: bool,
        ctx: &mut ViewContext<Self>,
    ) -> Vec<ViewHandle<ActionButton>> {
        (0..UNCAGED_MAX_CONN_ROWS)
            .map(|index| {
                ctx.add_typed_action_view(move |_| {
                    let button = if is_use {
                        ActionButton::new("Use", SecondaryTheme)
                    } else {
                        ActionButton::new("Edit", SecondaryTheme).with_icon(Icon::Pencil)
                    };
                    button.with_size(ButtonSize::Small).on_click(move |ctx| {
                        ctx.dispatch_typed_action(if is_use {
                            AISettingsPageAction::UncagedUseConnection(index)
                        } else {
                            AISettingsPageAction::UncagedConfigureConnection(index)
                        });
                    })
                })
            })
    ...
    /// Connect a catalog preset. No-key local models and CLI agents go live
    ...
    fn uncaged_connect_preset(&mut self, preset_id: &str, ctx: &mut ViewContext<Self>) {
        match crate::uncaged::connect(preset_id) {
            Ok(id) => match crate::uncaged::connections().into_iter().find(|c| c.id == id) {
                ...
                Some(conn) => self.open_uncaged_connection_editor(&conn, ctx),
                None => ctx.notify(),
            },
            Err(err) => self.uncaged_toast(err, ctx),
        }
    }

    /// Make the roster connection at `index` the active one.
    fn uncaged_use_connection(&mut self, index: usize, ctx: &mut ViewContext<Self>) {
        let conns = crate::uncaged::connections();
        let Some(conn) = conns.get(index) else {
            return;
        };
        match crate::uncaged::activate(&conn.id) {
            Ok(()) => {
                self.uncaged_toast(format!("Now using {}.", conn.label), ctx);
                ctx.notify();
            }
            Err(err) => self.uncaged_toast(err, ctx),
        }
    ...
    /// Open the editor for the roster connection at `index`.
    fn uncaged_configure_connection(&mut self, index: usize, ctx: &mut ViewContext<Self>) {
        let conns = crate::uncaged::connections();
        let Some(conn) = conns.get(index).cloned() else {
            return;
        };
        self.open_uncaged_connection_editor(&conn, ctx);
    }

    /// Open the shared endpoint modal prefilled from a roster connection, and
    /// arm `pending_uncaged_connection_id` so its save/remove route to the
    /// roster rather than Warp's custom endpoints.
    fn open_uncaged_connection_editor(
        &mut self,
        conn: &crate::uncaged::ConnectionView,
        ctx: &mut ViewContext<Self>,
    ) {
        self.pending_uncaged_connection_id = Some(conn.id.clone());
        // A CLI agent has no URL/key — its lever is the command line, so put the
        // command into the field the modal shows (relabeled "Command" below) and
        // tell the modal it's a CLI so it hides the key and skips URL validation.
        let is_cli = conn.wire == "cli";
        let kind = if is_cli {
            EndpointKind::Cli
        } else {
            EndpointKind::Api
        };
        let synthetic = ::ai::api_keys::CustomEndpoint {
            name: conn.label.clone(),
            url: if is_cli {
                conn.cli_command.join(" ")
            } else {
                conn.base_url.clone()
            },
            api_key: String::new(),
            models: vec![::ai::api_keys::CustomEndpointModel {
                name: conn.model.clone(),
                alias: None,
                config_key: String::new(),
            }],
        };
        self.custom_endpoint_modal_state
            .set_title(Some(format!("Connect {}", conn.label)), ctx);
        // Set the kind before prefilling so the URL placeholder/validation match.
        self.custom_endpoint_modal_state.set_kind(kind, ctx);
        self.custom_endpoint_modal_state
            .prefill(Some(&synthetic), Some(0), ctx);
        self.custom_endpoint_modal_state.open(ctx);
        ctx.emit(AISettingsPageEvent::ShowModal);
        ctx.notify();
    }

    /// Persist edits from the modal back onto a roster connection and activate
    /// it. A blank key means "keep whatever is stored".
    fn commit_uncaged_connection(
        &mut self,
        id: &str,
        name: &str,
        url: &str,
        ...
        let is_cli = crate::uncaged::connections()
            .iter()
            .find(|c| c.id == id)
            .map(|c| c.wire == "cli")
            .unwrap_or(false);
        let (base_url, cli_command) = if is_cli {
            (
                String::new(),
    ...
    /// Remove the pending roster connection (from the editor's Remove button).
    fn remove_uncaged_connection(&mut self, id: &str, ctx: &mut ViewContext<Self>) {
        let result = crate::uncaged::remove(id);
        self.hide_custom_endpoint_modal(ctx);
        match result {
            Ok(()) => self.uncaged_toast("Model removed.".to_string(), ctx),
            Err(err) => self.uncaged_toast(err, ctx),
        }
        ctx.notify();
    }

    fn uncaged_toast(&mut self, message: String, ctx: &mut ViewContext<Self>) {
        let window_id = ctx.window_id();
        crate::ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
            let toast = crate::view_components::DismissibleToast::success(message);
            toast_stack.add_ephemeral_toast(toast, window_id, ctx);
        });
    }
    ...
    fn hide_custom_endpoint_modal(&mut self, ctx: &mut ViewContext<Self>) {
        // Closing the modal ends any in-progress Uncaged connection edit.
        self.pending_uncaged_connection_id = None;
    ...
    fn handle_custom_endpoint_modal_event(
        ...
        match event {
            ...
            CustomEndpointModalEvent::AddEndpoint {
                ...
                // When configuring an Uncaged connection, route to the roster
                // instead of Warp's custom endpoints.
                if let Some(id) = self.pending_uncaged_connection_id.clone() {
                    self.commit_uncaged_connection(&id, name, url, api_key, models, ctx);
                    return;
                }
                if !Self::can_use_custom_inference_controls(ctx) {
                    self.hide_custom_endpoint_modal(ctx);
            ...
            CustomEndpointModalEvent::RemoveEndpoint { index } => {
                if let Some(id) = self.pending_uncaged_connection_id.clone() {
                    self.remove_uncaged_connection(&id, ctx);
                    return;
                }
                if !Self::can_use_custom_inference_controls(ctx) {
                    self.hide_custom_endpoint_modal(ctx);
    ...
    fn build_page(subpage: Option<AISubpage>, ctx: &mut ViewContext<Self>) -> PageType<Self> {
        ...
        match subpage {
            None => {
                ...
                widgets.push(Box::new(ConnectModelWidget));
...
pub enum AISettingsPageAction {
    ...
    // Uncaged "Connect a model"
    UncagedConnectPreset(String),
    UncagedUseConnection(usize),
    UncagedConfigureConnection(usize),
...
impl TypedActionView for AISettingsPageView {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            AISettingsPageAction::UncagedConnectPreset(preset_id) => {
                self.uncaged_connect_preset(preset_id, ctx);
            }
            AISettingsPageAction::UncagedUseConnection(index) => {
                self.uncaged_use_connection(*index, ctx);
            }
            AISettingsPageAction::UncagedConfigureConnection(index) => {
                self.uncaged_configure_connection(*index, ctx);
            }
...
impl SettingsWidget for GlobalAIWidget {
    ...
    fn render(
        ...
        // Uncaged: never nag to create an account when the local engine powers AI.
        // Uncaged has no accounts — AI is always available via the local engine,
        // so this is just the global AI on/off toggle, never a sign-up nag.
        let _ = is_anonymous;
        row.add_child(
            Container::new(
                ui_builder
...
impl AgentsWidget {
    ...
    fn render_base_model_setting(
        ...
        render_dropdown_item(
            ...
            "Base model",
            Some(
                "This model serves as the primary engine behind the agent. It powers most interactions and invokes other models for tasks like planning or code generation when necessary. Uncaged may automatically switch to alternate models based on model availability or for auxiliary tasks such as conversation summarization.",
            ),
    ...
    fn render_codebase_context_outline_generation_setting(
        ...
        let codebase_context_description = vec![
            FormattedTextFragment::plain_text(
                "Allow the agent to generate an outline of your codebase that can be used for context. No code is ever stored on our servers. ",
            ),
...
impl VoiceWidget {
    fn render_voice_section(
        ...
        let voice_input_description_text_fragments = vec![
            FormattedTextFragment::plain_text(
                "Voice input allows you to control Uncaged by speaking directly to your terminal (powered by ",
            ),
...
impl SettingsWidget for CLIAgentWidget {
    ...
    fn render(
        ...
        // Uncaged: list which third-party CLI agents are actually installed on
        // this machine, so they "show up" rather than being an invisible config.
        // Driven by CLIAgentInstallModel — a local PATH scan, no network, no
        // subprocess. The scan runs at startup, so by the time this page opens it
        // has usually completed (a brief "Checking…" self-corrects on next paint).
        {
            use crate::terminal::cli_agent::{CLIAgent, CLIAgentInstallModel};

            let theme = appearance.theme();
            let install = CLIAgentInstallModel::as_ref(app);
            let scanned = install.is_scan_complete();

            let mut detected = Flex::column().with_child(
                Container::new(
                    build_sub_header(
                        appearance,
                ...
                .enumerate()
            {
                ...
                if let Some(icon) = agent.icon() {
                    row = row.with_child(
                        ConstrainedBox::new(
                            icon.to_warpui_icon(theme.active_ui_text_color()).finish(),
                        )
                        .with_width(16.)
                        .with_height(16.)
                        .finish(),
            ...
            column.add_child(detected.finish());
        }
                ...
                let auto_show_toggle_label = render_body_item_label::<AISettingsPageAction>(
                    ...
                    Some(AdditionalInfo {
                        ...
                        tooltip_override_text: Some(
                            "Requires the Uncaged plugin for your coding agent".to_owned(),
                        ),
                    }),
                    LocalOnlyIconState::for_setting(
...
impl SettingsWidget for AgentAttributionWidget {
    ...
    fn render(
        ...
        Flex::column()
            ...
            .with_child(render_ai_setting_description(
                "Uncaged can add attribution to commit messages and pull requests it creates",
                !state.is_disabled,
                app,
            ))
            .finish()
...
impl SettingsWidget for CloudAgentComputerUseWidget {
    ...
    fn render(
        ...
        Flex::column()
            ...
            .with_child(toggle_row)
            .with_child(render_ai_setting_description(
                "Enable computer use in cloud agent conversations started from Uncaged.",
                !is_disabled,
                app,
            ))
            .finish()
...
/// The Uncaged "Connect a model" section: the roster of saved connections
/// (one active, powering Agent Mode) plus a grouped gallery of every platform
/// you can connect — a local agent CLI, an on-device model, or an API key.
/// This is the always-visible face of the local bring-your-own-model engine.
struct ConnectModelWidget;

impl ConnectModelWidget {
    /// Row / tile title (13px semibold) — one tier above the muted subtitle.
    fn title(text: &str, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        Text::new(text.to_string(), appearance.ui_font_family(), 13.)
            .with_color(styles::header_font_color(true, app).into())
            .with_style(Properties::default().weight(Weight::Semibold))
            .finish()
    }
    ...
    fn muted(text: &str, size: f32, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        Text::new(text.to_string(), appearance.ui_font_family(), size)
            .with_color(styles::description_font_color(true, app).into())
            .finish()
    }

    /// A pill-shaped badge: 10px semibold colored text on a subtle rounded
    /// fill, so "Active"/"Ready"/"Local" read as a class of element.
    fn pill(text: &str, text_color: ThemeFill, appearance: &Appearance) -> Box<dyn Element> {
        Container::new(
        ...
        .with_horizontal_padding(7.)
        .with_vertical_padding(3.)
        .with_background(internal_colors::fg_overlay_2(appearance.theme()))
        .with_corner_radius(CornerRadius::with_all(Radius::Pixels(5.)))
        .finish()
    }

    /// The brand mark for a provider preset. Real vendor logos where they exist;
    /// a generic glyph for "custom". Centralized in `app/src/uncaged.rs` so the
    /// gallery and the AI-reply avatar always agree.
    fn preset_icon(preset: &str) -> Icon {
        crate::uncaged::preset_icon(preset)
    }

    /// A fixed-size leading brand icon. Icons have no intrinsic size, so they
    /// must be boxed in a `ConstrainedBox`. Rendered at a consistent neutral
    /// color in every state (a vendor logo should never be greyed out).
    fn leading_icon(preset: &str, appearance: &Appearance) -> Box<dyn Element> {
        ConstrainedBox::new(
            Self::preset_icon(preset)
                .to_warpui_icon(appearance.theme().active_ui_text_color())
                .finish(),
        )
        .with_width(UNCAGED_ICON_SIZE)
        .with_height(UNCAGED_ICON_SIZE)
        .finish()
    }

    /// A styled card wrapper for a connection row / gallery tile. The active
    /// connection gets an accent border to signal "this powers Agent Mode".
    ...
    fn connection_row(
        ...
        let mut name_row = Flex::row()
            .with_cross_axis_alignment(CrossAxisAlignment::Center)
            .with_spacing(6.)
            .with_child(Self::title(&conn.label, appearance, app));
        if conn.is_active {
        ...
        let left = Flex::column()
            ...
            .with_child(Self::muted(&subtitle_text, 11., appearance, app))
            .finish();

        let status_color = ThemeFill::Solid(if conn.status == "Ready" {
            theme.ansi_fg_green()
        } else {
        ...
        if !conn.is_active && conn.usable {
            if let Some(button) = view.uncaged_use_buttons.get(index) {
                right = right.with_child(button.as_ref(app).render(app));
            }
        }
        if let Some(button) = view.uncaged_edit_buttons.get(index) {
            right = right.with_child(button.as_ref(app).render(app));
        }

        // Icon + shrinkable text + fixed right controls, all DIRECT children of a
        // MainAxisSize::Max row. The Shrinkable fills the row (pushing controls to
        // the edge) and truncates long endpoints. Do NOT nest the Shrinkable inside
        // an intermediate sub-row — that sub-row is measured with an infinite width
        // constraint and the flex panics.
        let row = Flex::row()
            .with_main_axis_size(MainAxisSize::Max)
            .with_cross_axis_alignment(CrossAxisAlignment::Center)
            .with_spacing(11.)
            .with_child(Self::leading_icon(&conn.preset, appearance))
    ...
    fn render_gallery(
        ...
        appearance: &Appearance,
        app: &AppContext,
        connections: &[crate::uncaged::ConnectionView],
    ) -> Box<dyn Element> {
        let theme = appearance.theme();
        // Reflect connection state so the gallery doesn't re-offer "Connect" for
        // something already set up.
        let connected: std::collections::HashSet<&str> =
            connections.iter().map(|c| c.preset.as_str()).collect();
        let active_preset: Option<&str> = connections
            .iter()
            .find(|c| c.is_active)
            .map(|c| c.preset.as_str());

        let mut column = Flex::column().with_spacing(8.);
        // Index into `uncaged_connect_buttons`, built by flattening the catalog
        // in this same order. Advanced unconditionally so the positional button
        // pool never desyncs.
        let mut preset_idx = 0usize;
        for section in crate::uncaged::catalog_sections() {
            column.add_child(
                Container::new(
                    Flex::column()
                        .with_spacing(2.)
                        .with_child(Self::label(&section.title, appearance, app))
                        .with_child(Self::muted(
                            &section.subtitle,
                            CONTENT_FONT_SIZE,
                            appearance,
                            app,
            ...
            for preset in &section.presets {
                ...
                // Title only — the section header already states the pill copy
                // ("Your login"/"On-device"/"API key"); the right slot carries state.
                let name_row = Flex::row()
                    .with_cross_axis_alignment(CrossAxisAlignment::Center)
                    .with_spacing(6.)
                    .with_child(Self::title(&preset.label, appearance, app))
                    .finish();
                let left = Flex::column()
                    .with_spacing(3.)
                    .with_child(name_row)
                    .with_child(Self::muted(&preset.blurb, 11., appearance, app))
                    .finish();

                // Stable right column: a state pill when connected, else Connect.
                ...
                // Flat row (see connection_row): Shrinkable must be a direct child
                // of this MainAxisSize::Max row, not nested in a sub-row.
                let row = Flex::row()
                    .with_main_axis_size(MainAxisSize::Max)
                    .with_cross_axis_alignment(CrossAxisAlignment::Center)
                    .with_spacing(11.)
                    .with_child(Self::leading_icon(&preset.id, appearance))
                    .with_child(Shrinkable::new(1., left).finish())
                    .with_child(right)
                    .finish();

                column.add_child(Self::card(row, is_active, appearance));
            }
        }
        column.finish()
    }
...
impl SettingsWidget for ConnectModelWidget {
    type View = AISettingsPageView;

    fn search_terms(&self) -> &str {
        "connect a model ai local agent cli claude code codex gemini ollama lm studio api key \
         anthropic openai openrouter google groq deepseek mistral xai grok together byo bring your own"
    }
    ...
    fn render(
        ...
        column.add_child(
            Container::new(Self::muted(
                "Power Agent Mode with your own model — a local agent CLI you're already signed in \
                 to, a private on-device model (Ollama / LM Studio), or any API key. Nothing goes \
                 to Uncaged; your keys stay on this machine.",
                CONTENT_FONT_SIZE,
                appearance,
                app,
            ))
        ...
        // Fetch the roster once and share it with the gallery (avoids a second
        // disk read and keeps state consistent across both sections).
        let connections = crate::uncaged::connections();
        if !connections.is_empty() {
            column.add_child(
                Container::new(Self::label("Connected", appearance, app))
                    .with_margin_bottom(8.)
                    .finish(),
            );
        ...
        column.add_child(self.render_gallery(view, appearance, app, &connections));
...
impl ApiKeysWidget {
    ...
    fn render_custom_inference_description(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let text_fragments = vec![
            FormattedTextFragment::plain_text(
                "Use your own API keys from model providers for the agent. You can also add custom endpoints to use third-party models. Custom endpoints must support the OpenAI-compatible Chat Completions API. API keys are stored only on your device, never on our servers. They're used to make requests to your chosen model provider. ",
            ),
    ...
    fn render_custom_inference_info_icon(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let tooltip_text = FormattedText::new([FormattedTextLine::Line(vec![
            ...
            FormattedTextFragment::hyperlink("the Uncaged license", CUSTOM_INFERENCE_TERMS_URL),
            FormattedTextFragment::plain_text(
                ". Your API keys and requests are sent directly to the provider you configure — never to Uncaged.",
            ),
        ])]);
    ...
    fn render_grok_subscription_row(
        ...
        let description = Container::new(
            Text::new(
                "Connect your SuperGrok subscription to use Grok models in the agent through your xAI account.",
                appearance.ui_font_family(),
                CONTENT_FONT_SIZE,
            )
    ...
    fn render_warp_credit_fallback_toggle(
        ...
        let description = render_ai_setting_description(
            "When enabled, agent requests may be routed to one of Uncaged's provided models in the event of an error. Uncaged will prioritize using your API keys over your Uncaged credits.",
            ai_settings.is_any_ai_enabled(app),
            app,
        );
    ...
    fn render(
        ...
        if !is_byo_enabled {
                ...
                UserWorkspaces::as_ref(app).current_team()
            {
                ...
                // Uncaged: bringing your own API key is free and needs no
                // account or paid plan, so there's nothing to upgrade to.
                Vec::new()
            };
...
impl AwsBedrockWidget {
    ...
    fn render_aws_bedrock_section(
        ...
        let toggle_description = if is_admin_enforced {
            "Uncaged loads and sends local AWS CLI credentials for Bedrock-supported models. This setting is managed by your organization.".to_string()
        } else {
            "Uncaged loads and sends local AWS CLI credentials for Bedrock-supported models."
                .to_string()
        };
