...
impl SettingsWidget for AboutPageWidget {
    ...
    fn render(
        ...
        // LightOnDark = light content on a dark background (a dark theme), which
        // needs the light-art wordmark. That art lives in *-dark-title.svg (its
        // fill is #E9E1D4). The names describe the target background, so keep
        // this mapping and don't "fix" it back to matching-name branches.
        let image_path = if theme.inferred_color_scheme() == ColorScheme::LightOnDark {
            "bundled/svg/uncaged-logo-dark-title.svg"
        } else {
            "bundled/svg/uncaged-logo-light-title.svg"
        };

        // Release bundles carry a GIT_RELEASE_TAG; dev/local builds don't, so
        // fall back to the crate version (e.g. "v0.1.0") instead of showing a
        // "v#.##.###" placeholder.
        let version =
        ...
        Align::new(
            Flex::column()
                ...
                .with_child(
                    ui_builder
                        .span(crate::brand::ABOUT_DESCRIPTION)
                        .build()
                        .with_margin_top(16.)
                        .finish(),
                )
