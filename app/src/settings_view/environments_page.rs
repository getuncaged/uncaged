...
const PAGE_DESCRIPTION_TEXT: &str = "Environments define where your ambient agents run. Set one up in minutes via GitHub (recommended), Uncaged-assisted setup, or manual configuration.";
        ...
        let header = match list_scope {
            ...
            EnvironmentListScope::Team => {
                let shared_by_text = UserWorkspaces::as_ref(app)
                    .current_team()
                    .map(|team| format!("Shared by Uncaged and {}", team.name))
                    .unwrap_or_else(|| "Shared by Uncaged and your team".to_string());
...
impl SettingsPageMeta for EnvironmentsPageView {
    ...
    fn should_render(&self, _ctx: &AppContext) -> bool {
        return false; // Uncaged: hidden in the free, no-account fork
        true
    }
