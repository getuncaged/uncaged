            ...
            move |view, result, ctx| {
                ...
                match result {
                    Ok(()) => {
                        ...
                        let message = format!(
                            "Successfully installed the Uncaged Control CLI! You can now run '{command_name}' from the command line."
                        );
                    ...
                    Err(error) => {
                        let message = format!("Failed to install Uncaged Control command: {error}");
                        log::warn!("{message}");
                        ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
...
impl SettingsWidget for WarpControlCliInstallWidget {
    ...
    fn render(
        ...
        render_body_item::<ScriptingSettingsPageAction>(
            "Uncaged Control CLI command".into(),
            None,
...
impl SettingsWidget for LocalControlModeWidget {
    ...
    fn render(
        ...
        render_body_item::<ScriptingSettingsPageAction>(
            ...
            Some("warpctrl allows for scripting Uncaged's UI. Use with care.".to_owned()),
        )
    }
}
