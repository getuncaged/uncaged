...
No — not to any Uncaged or Warp servers. Uncaged never sends your prompts or terminal data to a central service, and never phones home. The only request it makes on its own initiative is to the model endpoint you configure yourself in **Settings → AI Models**. There are no accounts, no telemetry, no analytics, and no cloud sync.
